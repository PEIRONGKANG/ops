...
function Field({ disabled, errors, name, register, required }: {
  ...
  return <TextField disabled={disabled} error={Boolean(errors[name])} helperText={errors[name]?.message} label={fieldLabels[name]} {...register(name, { required: required ? '请填写此项。' : false })} />;
}
