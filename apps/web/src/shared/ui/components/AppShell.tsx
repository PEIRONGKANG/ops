...
import { AppBar, Badge, Box, IconButton, Popover, Stack, Toolbar, type SxProps, type Theme } from '@mui/material';
...
export function AppShell({ children, headerContent, mainSx, navigation, notificationContent, notificationCount = 0 }: AppShellProps) {
  const [notificationAnchor, setNotificationAnchor] = useState<HTMLElement | null>(null);
  const notificationOpen = Boolean(notificationAnchor);
  const mainStyles = Array.isArray(mainSx) ? mainSx : mainSx ? [mainSx] : [];
