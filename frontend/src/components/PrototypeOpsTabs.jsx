...
function statusClass(status) {
  ...
  if (["迟到", "库存预警", "异常待处理", "待票据"].includes(status)) return "risk";
  return "warning";
}
...
function buildFinanceRows(currentDayData, studentUsers) {
  ...
  return [
    ...
    { id: "FIN-DAY-LOSS", use: "异常损耗登记", owner: ownerA, amount: `¥ ${currentDayData?.lossAmount || 0}`, status: currentDayData?.lossAmount ? "待 P2 审批" : "待票据", next: "审批" },
...
export function PrototypeOpsTabs({ activeTab, currentDayData, studentUsers, onNavigate, onLoadWeek, onPreviewReport }) {
  ...
  const tasks = useMemo(() => [
    { step: 1, title: "学生签到并确认岗位", meta: "使用服务器时间签到/签退，异常需补充说明", status: currentDayData?.checkIn ? "done" : "risk", label: currentDayData?.checkIn ? "已完成" : "异常待处理", tab: "daily" },
    { step: 2, title: "开店前卫生与设备检查", meta: "吧台、公区、制冰机照片上传后等待 P2 确认", status: "pending", label: "待确认", tab: "daily" },
    { step: 3, title: "库存盘点与物料预警", meta: currentDayData?.inventoryDesc || "咖啡豆、鲜奶、杯盖低于安全线，建议补货", status: "risk", label: "库存预警", tab: "prototype_inventory" },
  ...
  const filteredTasks = tasks.filter((task) => {
    if (taskFilter === "risk") return task.status === "risk";
  ...
  if (activeTab === "prototype_attendance") {
    return (
      ...
      >
        <div className="prototype-table-wrap"><table className="prototype-table"><thead><tr><th>学生</th><th>岗位</th><th>签到</th><th>签退</th><th>状态</th><th>操作</th></tr></thead><tbody>{attendanceRows.map((row) => <tr key={`${row.name}-${row.role}`}><td><strong>{row.name}</strong></td><td>{row.role}</td><td>{row.in}</td><td>{row.out}</td><td><Tag status={statusClass(row.state)}>{row.state}</Tag></td><td><button className="prototype-line-button" onClick={() => onNavigate?.("daily")}>{row.next}</button></td></tr>)}</tbody></table></div>
      </Panel>
    );
  }
  ...
  if (activeTab === "prototype_inventory") {
    return (
      <Panel
        eyebrow="库存管理"
        title="物料安全线"
        actions={<><input className="prototype-search" value={inventoryQuery} onChange={(e) => setInventoryQuery(e.target.value)} placeholder="搜索物料或仓位" /><select className="prototype-select" value={inventoryFilter} onChange={(e) => setInventoryFilter(e.target.value)}><option value="all">全部库存</option><option value="risk">仅看预警</option><option value="normal">库存正常</option></select></>}
      >
        <div className="prototype-inventory-list">{inventoryRows.map((row) => { const percent = Math.min(100, Math.round((row.current / row.safe) * 100)); return <article key={row.item} className={`prototype-inventory-card ${row.risk ? "risk" : ""}`}><div><strong>{row.item}</strong><p>{row.location} · 安全线 {row.safe}{row.unit}</p></div><div className="prototype-stock-bar"><span style={{ width: `${percent}%` }} /></div><button className={`prototype-data-tag ${row.risk ? "risk" : "normal"}`} onClick={() => onNavigate?.("daily")}>{row.current}{row.unit}</button></article>; })}</div>
      </Panel>
    );
  }
    ...
    return (
      <div className="prototype-split-grid">
        <Panel eyebrow="财务管理" title="费用审批与流水" actions={<button className="btn-primary" onClick={() => onNavigate?.("daily")}>新增费用</button>}>
          <div className="prototype-table-wrap"><table className="prototype-table"><thead><tr><th>单号</th><th>用途</th><th>申请人</th><th>金额</th><th>状态</th><th>下一步</th></tr></thead><tbody>{financeRows.map((row) => <tr key={row.id}><td><strong>{row.id}</strong></td><td>{row.use}</td><td>{row.owner}</td><td>{row.amount}</td><td><Tag status={statusClass(row.status)}>{row.status}</Tag></td><td><button className="prototype-line-button" onClick={() => onNavigate?.("daily")}>{row.next}</button></td></tr>)}</tbody></table></div>
        </Panel>
  ...
  if (activeTab === "prototype_students") {
    return (
      <Panel eyebrow="学生状态" title="岗位与产出" actions={<input className="prototype-search" value={studentQuery} onChange={(e) => setStudentQuery(e.target.value)} placeholder="搜索姓名、岗位、状态" />}>
        <div className="prototype-student-grid">{studentCards.map((student) => <article key={`${student.name}-${student.role}`} className="prototype-student-card"><h3>{student.name}</h3><p>{student.role} · 完成度 {student.progress}</p><div>{[student.status, ...student.tags].map((tag) => <Tag key={tag} status={statusClass(tag)}>{tag}</Tag>)}</div></article>)}</div>
      </Panel>
    );
  }
